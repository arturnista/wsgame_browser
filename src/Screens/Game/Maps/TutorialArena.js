...
function TutorialArena(data, { app, camera, parent }) {
    this.app = app
    this.parent = parent
    this.data = data
    ...
    const bg = new window.PIXI.extras.TilingSprite(window.textures['tutorial_arena_bg.png'], this.app.renderer.screen.width * 10, this.app.renderer.screen.height * 10)
    bg.anchor.set(.5, .5)
...
TutorialArena.prototype.updateData = function(data) {
    this.data = data
    this.sprite.x = this.data.position.x
    this.sprite.y = this.data.position.y
    this.currentSize = this.data.size
...
TutorialArena.prototype.updateMask = function(force) {   
